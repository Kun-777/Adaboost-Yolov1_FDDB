...
class WeakClassifier:
    ...
    def fit(self, X, y, w):
        ...
        W = np.tile(w.transpose(), (X.shape[1],1)).transpose()
        Y = np.tile(y.transpose(), (X.shape[1],1)).transpose()
        # sort each feature column and reorder the Weights and Labels in the same order
        sorted_idx = np.argsort(X, axis=0)
        sorted_X = np.take_along_axis(X, sorted_idx, axis=0)
        sorted_W = np.take_along_axis(W, sorted_idx, axis=0)
        sorted_Y = np.take_along_axis(Y, sorted_idx, axis=0)
        ...
        cum_w_error_left = np.cumsum(sorted_W * (sorted_Y ^ 1), axis=0)[:-1, :]
        cum_w_error_right = np.cumsum(sorted_W[::-1, :] * sorted_Y[::-1, :], axis=0)[::-1, :][1:, :]
        cum_w_error = cum_w_error_left + cum_w_error_right
        parity = np.ones(cum_w_error.shape)
        ...
        thres_idx, self.best_feature = np.unravel_index(cum_w_error.argmin(), cum_w_error.shape)
        self.parity = parity[thres_idx, self.best_feature]
        self.best_threshold = (sorted_X[thres_idx, self.best_feature] + sorted_X[thres_idx+1, self.best_feature]) / 2
        # for i in range(X.shape[1]):
        #     feature = X[:, i]
        #     thresholds = np.unique(feature)
        #     # try every threshold
        #     for threshold in thresholds:
        #         # Suppose we predict left of threshold to be class 1 and right to be class 0
        #         parity = 1
        #         e = (feature < threshold) ^ y
        #         error = (w*e).sum(0)
        #         if error > 0.5:
        #             # flip the decision
        #             e = 1 - e
    ...
    def fit(self, X_train, y_train, w, X_test=None, y_test=None):
        ...
        for _ in tqdm(range(self.T)):
            ...
            if X_test is not None:
                ...
                train_pred, _ = self.predict(X_train)
                test_pred, _ = self.predict(X_test)
                train_errors.append((np.abs(train_pred - y_train)).sum(0)/len(train_pred))
